...
public abstract class BaseMapServlet extends HttpServlet {
    ...
    protected synchronized MapPrinter getMapPrinter(String app) throws ServletException {
        ...
        String configDigMapPath = System.getProperty("digmap-config", getInitParameter("digmap-config"));
        configDigMapPath =getServletContext().getRealPath("/data/printing/digmap/digmap_config.properties");
        LOGGER.debug("REAL PATH1: +"+getServletContext().getRealPath("/"));
        LOGGER.debug("REAL PATH2: +"+getServletContext().getRealPath("/data/printing/digmap"));
        ...
        if (configDigMapPath == null) {        	
            throw new ServletException("Missing digmap configuration in web.xml 'web-app/servlet/init-param[param-name=digmap_config]' or 'web-app/context-param[param-name=digmap_config]'");
        }
        
        LOGGER.info("MFP config file: "+configPath);
        LOGGER.info("DM  config file: "+configDigMapPath);
        ...
        if (!configDigMapPath.toLowerCase().endsWith(".properties")) {
        	configDigMapPath += ".properties";
        }        

        File configFile = new File(app);
        File configDigMapFile = new File(configDigMapPath);
        ...
        if (!configDigMapFile.isAbsolute() || !configDigMapFile.exists()) {

            LOGGER.info("Attempting to locate DM config file: '" + configDigMapFile + " in the webapplication.");
            String realPath = getServletContext().getRealPath(configDigMapPath);

            if (realPath != null) {
            	configDigMapFile = new File(realPath);
            } else {
                LOGGER.info("Unable to find config file in web application using getRealPath.  Adding a / because that is often dropped");
                realPath = getServletContext().getRealPath("/" + configDigMapPath);
                configDigMapPath = realPath;
                configDigMapFile = new File(realPath);
            }
        }        
        ...
        if (printer == null) {
            ...
            try {
                ...
                //add DM config
                printer.getConfig().setConfigDM(configDigMapPath);
